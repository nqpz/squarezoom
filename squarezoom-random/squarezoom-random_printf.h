...
static void build_text(const struct lys_context *ctx, char* dest, size_t dest_len, const char* format, float render_milliseconds, char* **sum_names) {
  union { int32_t val; char* sum_name; } out0;
  union { int32_t val; char* sum_name; } out1;
  union { float val; char* sum_name; } out2;
  union { float val; char* sum_name; } out3;
  union { float val; char* sum_name; } out4;
  union { int32_t val; char* sum_name; } out5;
  ...
  snprintf(dest, dest_len, format, out0.sum_name, out1.sum_name, out2.val, out3.val, out4.val, out5.sum_name);
}
